...
# 创建滑动条
cv2.createTrackbar(
    "Var Color Threshold", "settings", var_color_threshold, 500, on_var_color_trackbar
)
cv2.createTrackbar(
...
while True:
    ...
    for i in range(image_gray.shape[0]):
        ...
        if (
            var_color[0] < var_color_threshold
            and var_color[1] < var_color_threshold
            and var_color[2] < var_color_threshold
        ):
            if previous_row_color is not None:
                ...
                if color_difference > color_difference_threshold:
                    cv2.line(
                        marked_image, (0, i), (marked_image.shape[1], i), (0, 0, 255), 2
                    )

            previous_row_color = average_color

    # 缩放标记后的图像用于展示
    resized_marked_image = cv2.resize(
        marked_image,
        (
            int(marked_image.shape[1] * scale_factor),
            int(marked_image.shape[0] * scale_factor),
        ),
    )
